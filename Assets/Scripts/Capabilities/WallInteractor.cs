...
public class WallInteractor : MonoBehaviour
{
    ...
    [Header("Wall Slide")]
    [SerializeField]
    [Range(0.1f, 5f)] private float _wallClimbMaxSpeed = 2f;
    [Header("Wall Jump")]
    ...
    private void FixedUpdate()
    {
        ...
        #region Wall Climb
        if (_onWall && _controller.input.RetrieveClimbInput(this.gameObject) < 0)
        {
            if (_velocity.y < -_wallClimbMaxSpeed)
            {
                _velocity.y = -_wallClimbMaxSpeed;
            }
        }
        else if (_onWall && _controller.input.RetrieveClimbInput(this.gameObject) > 0)
        {
            if (_velocity.y < _wallClimbMaxSpeed)
            {
                _velocity.y = _wallClimbMaxSpeed;
